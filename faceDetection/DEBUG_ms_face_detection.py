...
class APIFaceDetection(object):
    def __init__(self):
        ...
        if isfile('students.db') == False:
            print("Downloading students database...")
        ...
        while True: #webcam.isOpened():
            ...
            for idx, f in enumerate(face): 
                ...
                if time.time() - last_face_check < config.API_CALL_COOLDOWN:
                    ...
                    # write time to end cooldown         
                    cv2.putText(frame, str(config.API_CALL_COOLDOWN - (time.time() - last_face_check)), (0,len(frame) - 1), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                                (0,255,0), 2)
                    continue
                ...
                if frame_number == config.FRAMES_TO_ANALYZE:                
                    ...
                    if best_result * 100 > 80:
                        ...
                        try:
                            cv2.imshow("Detected face" + str(i), best_photo)
                        except:
                            print("Photo is too small")
                            print("StartY: ", startY)
                            print("EndY: ", endY)
                            print("StartX: ", startX)
                            print("EndX: ", endX)
                            print("---------------------------")
                            print("best_photo_startY: ", best_photo_startY)
                            print("best_photo_endY: ", best_photo_endY)
                            print("best_photo_startX: ", best_photo_startX)
                            print("best_photo_endX: ", best_photo_endX)
                        ...
                        # Information about detected face
                        print(json.dumps(student_info, indent=4, sort_keys=True ))
                        print("\n\n")
                        ...
                        print("Latencja: ", time.time() - last_face_check)                            
                        # Information about detected person
                        print(student_id.status_code)
            ...
            cv2.imshow("Real-time face detection based on Microsoft API", frame)
            # press "Q" to stop
